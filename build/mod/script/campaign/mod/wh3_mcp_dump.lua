...
local function seed_trigger_watermark()
...
local function seed_exec_watermark()
    local f = io.open(EXEC_TRIGGER_PATH, "r")
    if f then
        last_exec_trigger_time = tonumber(f:read("*a")) or 0
        f:close()
    end
end
...
local function run_exec_script()
    ...
    local res = {
        ...
        result = _G.wh3_exec_result,
        request = last_exec_trigger_time,
        timestamp = os.time(),
    }
    local enc_ok, encoded = pcall(json.encode, res)
    if enc_ok then
        write_file(EXEC_RESULT_PATH, encoded)
    else
        res.result = tostring(_G.wh3_exec_result)
        write_file(EXEC_RESULT_PATH, json.encode(res))
    end
...
cm:add_first_tick_callback(function()
    ...
    -- Flush events + check exec bridge periodically (fast poll, repeating)
    ...
    -- Periodic dumps (repeating; real_callback is single-shot)
    cm:repeat_real_callback(
        function()
            pcall(function()
                check_trigger()
